...
RUN apk --update --no-cache add argon2 argon2-dev curl supervisor \
    ...
    php7-openssl php7-pdo php7-mysqli php7-pdo_mysql php7-pdo_pgsql php7-pgsql php7-tokenizer \
...
RUN cp /etc/nginx/nginx.conf.tpl /etc/nginx/nginx.conf && \
    cp /etc/php7/php-fpm.d/www.conf.tpl /etc/php7/php-fpm.d/www.conf && \
